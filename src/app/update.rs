...
use super::{msg::{Cmd, Msg}, state::{ActivePanel, Model, Screen}};

pub fn update(mut model: Model, msg: Msg) -> (Model, Vec<Cmd>) {
    match msg {
        Msg::Tick => {
            if matches!(model.screen, Screen::IsoSearch) && !model.iso_searching && model.iso_results.is_empty() {
                model.iso_searching = true;
                let query = model.iso_query.clone();
                return (model, vec![Cmd::ScanIso { query }]);
            }
            if matches!(model.screen, Screen::DeviceSelect) && !model.device_refreshing && model.devices.is_empty() {
        ...
        Msg::Back => {
            match model.screen {
                Screen::IsoSearch => { /* top-level: no-op */ }
                Screen::DeviceSelect => {
        ...
        Msg::IsoResults(results) => {
            ...
            if model.iso_selected >= model.iso_results.len() { model.iso_selected = model.iso_results.len().saturating_sub(1); }
        ...
        Msg::DevicesRefreshed(devs) => {
            model.devices = devs;
            if model.device_selected >= model.devices.len() { model.device_selected = model.devices.len().saturating_sub(1); }
        ...
        Msg::WriteProgress { written, total, bps } => {
        ...
        Msg::WriteFinished(result) => {
            ...
            if model.write_result == Some(Ok(())) && model.verify_after_write {
                ...
                if let (Some(iso), Some(dev)) = (model.iso_chosen.clone(), model.device_chosen.clone()) {
                    return (model, vec![Cmd::Verify { iso, device: dev, size: total }]);
                }
            } else {
        ...
        Msg::VerifyProgress { checked, total, bps } => {
    ...
    match model.screen {
        Screen::IsoSearch => match key.code {
            ...
            KeyCode::Up => {
                let len = model.iso_results.len();
                if len > 0 {
                    if model.iso_selected > 0 { model.iso_selected -= 1; }
                }
            }
            KeyCode::Down => {
                let len = model.iso_results.len();
                if len > 0 {
                    if model.iso_selected + 1 < len { model.iso_selected += 1; }
                }
            }
            KeyCode::Enter => {
                if let Some(meta) = model.iso_results.get(model.iso_selected).cloned() {
            ...
            KeyCode::Backspace => {
                ...
                model.iso_selected = 0;
                model.iso_searching = true;
                return Some(Cmd::ScanIso { query: model.iso_query.clone() });
            }
            KeyCode::Char(c) => {
        ...
        Screen::DeviceSelect => match key.code {
            ...
            KeyCode::Up => {
                let len = model.devices.len();
                if len > 0 && model.device_selected > 0 { model.device_selected -= 1; }
            }
            KeyCode::Down => {
                let len = model.devices.len();
                if len > 0 && model.device_selected + 1 < len { model.device_selected += 1; }
            }
        ...
        Screen::Confirm => match key.code {
            ...
            KeyCode::Backspace => { model.confirm_input.pop(); }
            KeyCode::Enter => {
            ...
            KeyCode::Char('s') | KeyCode::Char('S') if key.modifiers.contains(KeyModifiers::CONTROL) => {
                if !model.is_root { return Some(Cmd::ReexecWithSudo); }
            }
...
fn next_screen(model: &Model) -> Screen {
    ...
    match model.screen {
        ...
        Screen::DeviceSelect => {
            if can_confirm { Screen::Confirm } else { Screen::IsoSearch }
        }
        Screen::Confirm => {
            Screen::IsoSearch
        }
...
fn prev_screen(model: &Model) -> Screen {
    ...
    match model.screen {
        Screen::IsoSearch => {
            if can_confirm { Screen::Confirm } else { Screen::DeviceSelect }
        }
