use ferric::app::state::{IsoMeta, Device, Model, Screen};
use ferric::app::msg::{Msg, Cmd};
...
fn test_iso_selection() {
    ...
    assert_eq!(new_model.iso_chosen, Some(PathBuf::from("/home/user/debian.iso")));
...
fn test_device_selection() {
    ...
    model.iso_chosen = Some(PathBuf::from("/home/user/test.iso"));
    model.devices = vec![
    ...
    assert_eq!(new_model.device_chosen, Some(PathBuf::from("/dev/sdb")));
...
fn test_write_lifecycle_with_verify() {
    ...
    model.iso_chosen = Some(PathBuf::from("/test.iso"));
    model.device_chosen = Some(PathBuf::from("/dev/sdb"));
    ...
    match &cmds[0] {
        Cmd::Verify { iso, device, size } => {
            assert_eq!(iso, &PathBuf::from("/test.iso"));
...
fn test_back_navigation() {
    // Back from IsoSearch is no-op (top level)
    let mut model = Model::default();
    model.screen = Screen::IsoSearch;
...
fn test_confirmation_requires_yes_input() {
    ...
    let (new_model, cmds) = update(model.clone(), Msg::Key(
    ...
    model.confirm_input = "YES".to_string();
    let (new_model, cmds) = update(model, Msg::Key(
        ratatui::crossterm::event::KeyEvent {
            code: ratatui::crossterm::event::KeyCode::Enter,
            modifiers: ratatui::crossterm::event::KeyModifiers::empty(),
            kind: ratatui::crossterm::event::KeyEventKind::Press,
